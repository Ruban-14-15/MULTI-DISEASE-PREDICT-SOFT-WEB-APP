...
diabetes_model = pickle.load(open("F:\\project\\Project\\MDPS\Model\\diabetes_model.sav", "rb"))

heart_disease_model = pickle.load(open("F:\\project\\Project\\MDPS\\Model\\heart_disease_model.sav", "rb"))

parkinsons_model = pickle.load(open("F:\\project\\Project\\MDPS\\Model\\parkinsons_model.sav", "rb"))

breast_cancer_model = pickle.load(open("F:\\project\\Project\\MDPS\\Model\\breast_cancer_model.sav", "rb"))

lung_cancer_model = pickle.load(open("F:\\project\\Project\\MDPS\\Model\\lung_cancer_model.sav", "rb"))
...
with st.sidebar:
    ...
    selected = option_menu("Multi-Disease Predict Soft-web App", 
                           
                           ["Diabetes Prognosis",
                            "Heart Disese Prognosis",
                            "Parkinsons Disease Prognosis",
                            "Breast Cancer Prognosis",
                            "Lung Cancer Prognosis",
                            "Kidney disease Prognois"],
                           
                           icons = ["activity", "heart-fill", "people-fill", 
                                    "gender-female", "apple","kidney"],
...
if(selected == "Diabetes Prognosis"):
    
    #page title
    st.title("Diabalytics: Insightful Prognosis")
...
if(selected == "Heart Disese Prognosis"):
    
    #page title
    st.title("CardioCare: Prognostic Precision")
...
if(selected == "Parkinsons Disease Prognosis"):
    
    #page title
    st.title("Parkinson's Insight: Forecasting")
...
if(selected == "Breast Cancer Prognosis"):
    
    #page title
    st.title("CancerGuard: Anticipating Breast Cancer")
...
if(selected == "Lung Cancer Prognosis"):
    
    #page title
    st.title("PulmoPredict: Forecasting Lung Cancer")
...
# code for Prediction
    ...
    if st.button("Lung Cancer Test Result"):
        lung_cancer_report = lung_cancer_model.predict([[AGE, SMOKING, YELLOW_FINGERS, ANXIETY, PEER_PRESSURE, CHRONIC_DISEASE, FATIGUE, ALLERGY, WHEEZING, ALCOHOL_CONSUMING, COUGHING, SHORTNESS_OF_BREATH, SWALLOWING_DIFFICULTY, CHEST_PAIN]])
...
# Define footer style based on theme
if theme == "#FFFFFF":  # Light mode
    footer_bg_color = "#f1f1f1"
    text_color = "#000000"
else:  # Dark mode
    footer_bg_color = "#383838"
    text_color = "#FFFFFF"
